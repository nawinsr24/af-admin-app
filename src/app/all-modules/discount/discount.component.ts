...
export class DiscountComponent implements OnInit {
  ...
  sizeForm = new FormGroup({
    ...
    discount_percentage: new FormControl('', [Validators.required, Validators.pattern(/^(?:\d|([1-9]\d)|99)$/)]),
  ...
  addDiscount() {
    ...
    this.productAPI.addDiscount({ discounts: [this.sizeForm.value] }).subscribe(res => {
  ...
  discountCalculation(event: any) {
    if (event.target.value > 99) return this.toaster.error("Invalid Discount !")
  ...
  async onCustomAction(event: any) {
    ...
    if (event.action == 'delete') {
      let r = await this.swal.deleteItem(this.productAPI.deleteDiscount(event.data.id))
      if (r) this.ngOnInit()
    }
  }
}
